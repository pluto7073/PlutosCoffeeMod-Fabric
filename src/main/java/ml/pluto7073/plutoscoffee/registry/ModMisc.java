...
public class ModMisc {
    ...
    public static <S extends RecipeSerializer<T>, T extends Recipe<?>> S registerRecipeSerializer(String id, S serializer) {
        return Registry.register(Registry.RECIPE_SERIALIZER, new Identifier(PlutosCoffee.MOD_ID, id), serializer);
    }

    static {
        PC_GROUP = FabricItemGroupBuilder.create(new Identifier(PlutosCoffee.MOD_ID, "pc_group"))
                .icon(() -> new ItemStack(ModItems.MEDIUM_ROAST_BEAN)).appendItems((stacks -> {
        ...
        COFFEE_WORK_RECIPE_TYPE = Registry.register(Registry.RECIPE_TYPE, new Identifier(PlutosCoffee.MOD_ID, "coffee_workstation"), new RecipeType<CoffeeWorkstationRecipe>() {
